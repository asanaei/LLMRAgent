    ...
    #'
    #' @param model_config LLMR model config used as default for chat and fallback summarization.
    #' @param summarizer_model_config Optional LLMR model config used specifically for summarization.
    ...
    #' @examples
    #' qa <- new_contextqa_agent(model_config = list(provider="openai", model="gpt-4o-mini"))
    ...
    #'
    #' Factory for a classifier agent that outputs a JSON object with fields
    ...
    #' @return An agent environment.
    #' @examples
    #' clf <- new_classifier_agent(list(provider="openai", model="gpt-4o-mini"), labels = c("positive","negative"))
    #' umsg <- build_classification_prompt("The movie was great!", c("positive","negative"))
